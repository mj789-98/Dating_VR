...
handlers.completedLevel = function (args, context) {
  ...
  log.debug(
    "Set lastLevelCompleted for player " + currentPlayerId + " to " + level
  );
  var request = {
    PlayFabId: currentPlayerId,
    Statistics: [
  ...
  log.debug(
    "Updated level_monster_kills stat for player " +
      currentPlayerId +
      " to " +
      monstersKilled
  );
...
function processPlayerMove(playerMove) {
  ...
  if (lastMoveTimestampSetting) {
    ...
    if (timeSinceLastMoveInSeconds < playerMoveCooldownInSeconds) {
      log.error(
        "Invalid move - time since last move: " +
          timeSinceLastMoveInSeconds +
          "s less than minimum of " +
  ...
  for (var i = 0; i < playerStats.length; i++)
    if (playerStats[i].StatisticName === "") movesMade = playerStats[i].Value;
  movesMade += 1;
  var request = {
    PlayFabId: currentPlayerId,
    Statistics: [
...
handlers.unlockHighSkillContent = function (args, context) {
  ...
  log.info(
    "Unlocked HighSkillContent for " + context.playerProfile.DisplayName
  );
...
handlers.RoomJoined = function (args) {
  log.debug(
    "Room Joined - Game: " + args.GameId + " PlayFabId: " + args.UserId
  );
};
...
handlers.GetAllUsers = function (args, context) {
  try {
    var users = [];
    var request = {
    ...
    function getUsersPage(skip) {
      request.Skip = skip;
      server.GetPlayers(request, function (err, result) {
        if (err) {
        ...
        result.data.Players.forEach(function (player) {
          users.push({
            ...
            DisplayName: player.DisplayName || "N/A",
            // Add other desired properties here (e.g., Created, LastLogin, etc.)
          });
        });

        if (result.data.Players.length === request.MaxResultsPerPage) {
          getUsersPage(skip + request.MaxResultsPerPage);
        } else {
          return context.success({ Users: users });
        }
      });
    }
    ...
    return context.failure({ Error: error.message });
  }
};
